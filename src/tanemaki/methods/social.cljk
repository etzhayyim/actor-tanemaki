(ns tanemaki.methods.social
  "social.cljc — tanemaki DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from tanemaki's own
...
(defn draft-observation-post
  "A dry-run observation post over tanemaki's own domain (Public Fund grant-steward (fund-manager inversion) — the SOWER: fuses DISCLOSED public evidence from the observatory lineage (kanjō 勘定…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "tanemaki R0: live social posting is Council Lv6+ + operator + "
