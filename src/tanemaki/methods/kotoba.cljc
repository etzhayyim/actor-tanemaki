#!/usr/bin/env bb
;; tanemaki 種蒔き — grant-stewardship graph DATOM-LOG writer (content-addressed commit-DAG).
(ns tanemaki.methods.kotoba
  "kotoba.cljc — tanemaki 種蒔き append-only STEWARDSHIP LEDGER writer
  (ADR-2606122001, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the inochi/asobi/tokigusuri/tate/ugachi/busshi family — the
  datomic-isomorphic canonical-state layer tanemaki previously only EMITTED
  (datom_emit) but never PERSISTED).

  Persists tanemaki's GROUND datoms — the screen / criterion / org / source / instrument /
  milestone NODES and their :en/* 縁 (the PUBLIC DD trail + disclosed rubric weights, G1) —
  to a local append-only EDN log of content-addressed transactions. DERIVED state (dd-fit /
  evidence-coverage / route, computed on READ, N1/G4) is NEVER stored: there is no stored
  org score and no stored decision.
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; tanemaki 種蒔き — STEWARDSHIP LEDGER (append-only, content-addressed EAVT "
                        "commit-DAG of the PUBLIC grant-DD screen/criterion/org/instrument graph). Generated; "
                        "DO NOT hand-edit. A steward's PUBLIC scorecard trail — never a decision (1 SBT = 1 "
                        "vote decides), never an investment. ADR-2606122001 / ADR-2605312345.\n")))
